...
import { Search, Check, Download, Upload, X, Play, ArrowRight, Edit3, Database, Zap, Settings, RefreshCw } from 'lucide-react';
...
export function ScanPane({ showToast, showOverlay, hideOverlay, onDraft }: any) {
...
export function VSPane({ showToast }: any) {
...
export function DraftsPane({ showToast, showOverlay, hideOverlay, selectedPost }: any) {
  const [drafts, setDrafts] = useState<any[]>([]);
  const [prompt, setPrompt] = useState('');
  const [style, setStyle] = useState('Formal Academic (PDF)');
  const [showQueries, setShowQueries] = useState(false);
  const [selectedBuckets, setSelectedBuckets] = useState<string[]>(['CS 401 / Week 4']);
...
export function KBPane({ showToast }: any) {
  ...
  const buckets = Array.from(new Set(items.map(i => i.bucket))) as string[];
...
export function AgentPane({ showToast, showOverlay, hideOverlay }: any) {
  const [logs, setLogs] = useState<string[]>(['No session activity yet.']);
...
export function SettingsPane({ showToast }: any) {
  return (
    <div className="p-4 pt-4">
