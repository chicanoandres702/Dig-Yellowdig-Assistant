function Readability(e, t) {
  ...
  if (t = t || {}, this._doc = e, this._docJSDOMParser = this._doc.firstChild.__JSDOMParser__, this._articleTitle = null, this._articleByline = null, this._articleDir = null, this._articleSiteName = null, this._attempts = [], this._metadata = {}, this._debug = !!t.debug, this._maxElemsToParse = t.maxElemsToParse || this.DEFAULT_MAX_ELEMS_TO_PARSE, this._nbTopCandidates = t.nbTopCandidates || this.DEFAULT_N_TOP_CANDIDATES, this._charThreshold = t.charThreshold || this.DEFAULT_CHAR_THRESHOLD, this._classesToPreserve = this.CLASSES_TO_PRESERVE.concat(t.classesToPreserve || []), this._keepClasses = !!t.keepClasses, this._serializer = t.serializer || function (e) {
      return e.innerHTML
    }, this._disableJSONLD = !!t.disableJSONLD, this._allowedVideoRegex = t.allowedVideoRegex || this.REGEXPS.videos, this._linkDensityModifier = t.linkDensityModifier || 0, this._flags = this.FLAG_STRIP_UNLIKELYS | this.FLAG_WEIGHT_CLASSES | this.FLAG_CLEAN_CONDITIONALLY, this._debug) {
    let e = function (e) {
...
Readability.prototype = {
  ...
  _getAllNodesWithTag: (e, t) => e.querySelectorAll ? e.querySelectorAll(t.join(",")) : [].concat.apply([], t.map((function (t) {
    var i = e.getElementsByTagName(t);
    return Array.isArray(i) ? i : Array.from(i)
  }))),
  _cleanClasses(e) {
    var t = this._classesToPreserve,
      i = (e.getAttribute("class") || "").split(/\s+/).filter((e => t.includes(e))).join(" ");
  ...
  _fixRelativeUris(e) {
    ...
    this._forEachNode(s, (function (e) {
      ...
      if (t && e.setAttribute("src", a(t)), i && e.setAttribute("poster", a(i)), r) {
        var s = r.replace(this.REGEXPS.srcsetUrl, (function (e, t, i, r) {
          return a(t) + (i || "") + r
        }));
    ...
    for (var t = e; t;) {
      if (t.parentNode && ["DIV", "SECTION"].includes(t.tagName) && (!t.id || !t.id.startsWith("readability"))) {
        ...
        if (this._hasSingleTagInsideElement(t, "DIV") || this._hasSingleTagInsideElement(t, "SECTION")) {
          for (var i = t.children[0], a = 0; a < t.attributes.length; a++) i.setAttributeNode(t.attributes[a].cloneNode());
  ...
  _getArticleTitle() {
    ...
    const s = /\|\-–—\\\/>»/.source;
    if (new RegExp(`\\s[${s}]\\s`).test(t)) {
      a = /\s[\\\/>»]\s/.test(t);
      let e = Array.from(i.matchAll(new RegExp(`\\s[${s}]\\s`, "gi")));
      r(t = i.substring(0, e.pop().index)) < 3 && (t = i.replace(new RegExp(`^[^${s}]*[${s}]`, "gi"), ""))
    } else if (t.includes(": ")) {
      var n = this._getAllNodesWithTag(e, ["h1", "h2"]),
        l = t.trim();
      this._someNode(n, (function (e) {
        return e.textContent.trim() === l
    ...
    var h = r(t = t.trim().replace(this.REGEXPS.normalize, " "));
    return h <= 4 && (!a || h != r(i.replace(new RegExp(`\\s[${s}]\\s`, "g"), "")) - 1) && (t = i), t
  },
  _prepDocument() {
    var e = this._doc;
  ...
  _replaceBrs(e) {
    this._forEachNode(this._getAllNodesWithTag(e, ["br"]), (function (e) {
      ...
      if (i) {
        ...
        for (; r.lastChild && this._isWhitespace(r.lastChild);) r.lastChild.remove();
  ...
  _setNodeTag(e, t) {
    ...
    for (var a = 0; a < e.attributes.length; a++) i.setAttributeNode(e.attributes[a].cloneNode());
    return i
  },
  _prepArticle(e) {
    ...
    this._forEachNode(e.children, (function (e) {
      ...
      return 0 === this._getAllNodesWithTag(e, ["img", "embed", "object", "iframe"]).length && !this._getInnerText(e, !1)
    })), this._forEachNode(this._getAllNodesWithTag(e, ["br"]), (function (e) {
  ...
  _isValidByline(e, t) {
    var i = e.getAttribute("rel"),
      ...
      r = e.textContent.trim().length;
    return ("author" === i || a && a.includes("author") || this.REGEXPS.byline.test(t)) && !!r && r < 100
  },
  ...
  _grabArticle(e) {
    ...
    for (var a = e.innerHTML;;) {
      ...
      var r = this._flagIsActive(this.FLAG_STRIP_UNLIKELYS),
        s = [],
        n = this._doc.documentElement;
      let J = !0;
      for (; n;) {
        "HTML" === n.tagName && (this._articleLang = n.getAttribute("lang"));
        var l = n.className + " " + n.id;
        if (this._isProbablyVisible(n))
          if ("true" != n.getAttribute("aria-modal") || "dialog" != n.getAttribute("role"))
            if (this._articleByline || this._metadata.byline || !this._isValidByline(n, l))
              if (J && this._headerDuplicatesTitle(n)) this.log("Removing header: ", n.textContent.trim(), this._articleTitle.trim()), J = !1, n = this._removeAndGetNext(n);
              else {
                if (r) {
                  if (this.REGEXPS.unlikelyCandidates.test(l) && !this.REGEXPS.okMaybeItsACandidate.test(l) && !this._hasAncestorTag(n, "table") && !this._hasAncestorTag(n, "code") && "BODY" !== n.tagName && "A" !== n.tagName) {
                    this.log("Removing unlikely candidate - " + l), n = this._removeAndGetNext(n);
                    continue
                  }
                  if (this.UNLIKELY_ROLES.includes(n.getAttribute("role"))) {
                    this.log("Removing content with role " + n.getAttribute("role") + " - " + l), n = this._removeAndGetNext(n);
                    continue
                  }
                }
                if ("DIV" !== n.tagName && "SECTION" !== n.tagName && "HEADER" !== n.tagName && "H1" !== n.tagName && "H2" !== n.tagName && "H3" !== n.tagName && "H4" !== n.tagName && "H5" !== n.tagName && "H6" !== n.tagName || !this._isElementWithoutContent(n)) {
                  if (this.DEFAULT_TAGS_TO_SCORE.includes(n.tagName) && s.push(n), "DIV" === n.tagName) {
                    for (var o = null, h = n.firstChild; h;) {
                      var d = h.nextSibling;
                      if (this._isPhrasingContent(h)) null !== o ? o.appendChild(h) : this._isWhitespace(h) || (o = t.createElement("p"), n.replaceChild(o, h), o.appendChild(h));
                      else if (null !== o) {
                        for (; o.lastChild && this._isWhitespace(o.lastChild);) o.lastChild.remove();
                        o = null
                      }
                      h = d
                    }
                    if (this._hasSingleTagInsideElement(n, "P") && this._getLinkDensity(n) < .25) {
                      var c = n.children[0];
                      n.parentNode.replaceChild(c, n), n = c, s.push(n)
                    } else this._hasChildBlockElement(n) || (n = this._setNodeTag(n, "P"), s.push(n))
                  }
                  n = this._getNextNode(n)
                } else n = this._removeAndGetNext(n)
              }
        else {
          for (var g = this._getNextNode(n, !0), _ = this._getNextNode(n), m = null; _ && _ != g;) {
            var u = _.getAttribute("itemprop");
            if (u && u.includes("name")) {
              m = _;
              break
            }
            _ = this._getNextNode(_)
          }
          this._articleByline = (m ?? n).textContent.trim(), n = this._removeAndGetNext(n)
        } else n = this._removeAndGetNext(n);
        else this.log("Removing hidden node - " + l), n = this._removeAndGetNext(n)
      }
      var p = [];
      this._forEachNode(s, (function (e) {
        if (e.parentNode && void 0 !== e.parentNode.tagName) {
          ...
          if (!(t.length < 25)) {
            ...
            if (0 !== i.length) {
              var a = 0;
              a += 1, a += t.split(this.REGEXPS.commas).length, a += Math.min(Math.floor(t.length / 100), 3), this._forEachNode(i, (function (e, t) {
                if (e.tagName && e.parentNode && void 0 !== e.parentNode.tagName) {
                  if (void 0 === e.readability && (this._initializeNode(e), p.push(e)), 0 === t) var i = 1;
                  else i = 1 === t ? 2 : 3 * t;
      ...
      for (var N = [], f = 0, E = p.length; f < E; f += 1) {
        var b = p[f],
          T = b.readability.contentScore * (1 - this._getLinkDensity(b));
        b.readability.contentScore = T, this.log("Candidate:", b, "with score " + T);
        for (var y = 0; y < this._nbTopCandidates; y++) {
          var A = N[y];
          if (!A || T > A.readability.contentScore) {
            N.splice(y, 0, b), N.length > this._nbTopCandidates && N.pop();
            break
          }
        }
      }
      var v, S = N[0] || null,
        C = !1;
      if (null === S || "BODY" === S.tagName) {
        for (S = t.createElement("DIV"), C = !0; e.firstChild;) this.log("Moving child out:", e.firstChild), S.appendChild(e.firstChild);
        e.appendChild(S), this._initializeNode(S)
      } else if (S) {
        for (var L = [], x = 1; x < N.length; x++) N[x].readability.contentScore / S.readability.contentScore >= .75 && L.push(this._getNodeAncestors(N[x]));
        if (L.length >= 3)
          for (v = S.parentNode;
            "BODY" !== v.tagName;) {
            for (var D = 0, I = 0; I < L.length && D < 3; I++) D += Number(L[I].includes(v));
            if (D >= 3) {
              S = v;
              break
            }
        ...
        S.readability || this._initializeNode(S), v = S.parentNode;
        for (var R = S.readability.contentScore, P = R / 3;
          "BODY" !== v.tagName;)
          if (v.readability) {
            var w = v.readability.contentScore;
            if (w < P) break;
            if (w > R) {
              S = v;
              break
            }
            R = v.readability.contentScore, v = v.parentNode
          } else v = v.parentNode;
        for (v = S.parentNode;
          "BODY" != v.tagName && 1 == v.children.length;) v = (S = v).parentNode;
        S.readability || this._initializeNode(S)
      }
      var O = t.createElement("DIV");
      i && (O.id = "readability-content");
      for (var B = Math.max(10, .2 * S.readability.contentScore), G = (v = S.parentNode).children, M = 0, H = G.length; M < H; M++) {
        var k = G[M],
          U = !1;
        if (this.log("Looking at sibling node:", k, k.readability ? "with score " + k.readability.contentScore : ""), this.log("Sibling has score", k.readability ? k.readability.contentScore : "Unknown"), k === S) U = !0;
        else {
          var W = 0;
          if (k.className === S.className && "" !== S.className && (W += .2 * S.readability.contentScore), k.readability && k.readability.contentScore + W >= B) U = !0;
          else if ("P" === k.nodeName) {
            var $ = this._getLinkDensity(k),
              F = this._getInnerText(k),
              X = F.length;
            (X > 80 && $ < .25 || X < 80 && X > 0 && 0 === $ && -1 !== F.search(/\.( |$)/)) && (U = !0)
          }
        }
        U && (this.log("Appending node:", k), this.ALTER_TO_DIV_EXCEPTIONS.includes(k.nodeName) || (this.log("Altering sibling:", k, "to div."), k = this._setNodeTag(k, "DIV")), O.appendChild(k), G = v.children, M -= 1, H -= 1)
      }
      if (this._debug && this.log("Article content pre-prep: " + O.innerHTML), this._prepArticle(O), this._debug && this.log("Article content post-prep: " + O.innerHTML), C) S.id = "readability-page-1", S.className = "page";
      else {
        var j = t.createElement("DIV");
        for (j.id = "readability-page-1", j.className = "page"; O.firstChild;) j.appendChild(O.firstChild);
        O.appendChild(j)
      }
      this._debug && this.log("Article content after paging: " + O.innerHTML);
      var V = !0,
        Y = this._getInnerText(O, !0).length;
      if (Y < this._charThreshold)
        if (V = !1, e.innerHTML = a, this._attempts.push({
            articleContent: O,
            textLength: Y
          }), this._flagIsActive(this.FLAG_STRIP_UNLIKELYS)) this._removeFlag(this.FLAG_STRIP_UNLIKELYS);
        ...
        if (this._attempts.sort((function (e, t) {
            return t.textLength - e.textLength
          })), !this._attempts[0].textLength) return null;
        O = this._attempts[0].articleContent, V = !0
      }
      if (V) {
        var z = [v, S].concat(this._getNodeAncestors(v));
        return this._someNode(z, (function (e) {
  ...
  _unescapeHtmlEntities(e) {
    ...
    return e.replace(/&(quot|amp|apos|lt|gt);/g, (function (e, i) {
      ...
      var a = parseInt(t || i, t ? 16 : 10);
      return (0 == a || a > 1114111 || a >= 55296 && a <= 57343) && (a = 65533), String.fromCodePoint(a)
    }))
  },
  _getJSONLD(e) {
    ...
    return this._forEachNode(i, (function (e) {
      if (!t && "application/ld+json" === e.getAttribute("type")) try {
        var i = e.textContent.replace(/^\s*<!\[CDATA\[|\]\]>\s*$/g, ""),
          a = JSON.parse(i);
        if (Array.isArray(a) && !(a = a.find((e => e["@type"] && e["@type"].match(this.REGEXPS.jsonLdArticleTypes))))) return;
        var r = /^https?\:\/\/schema\.org\/?$/;
        if (!("string" == typeof a["@context"] && a["@context"].match(r) || "object" == typeof a["@context"] && "string" == typeof a["@context"]["@vocab"] && a["@context"]["@vocab"].match(r))) return;
        if (!a["@type"] && Array.isArray(a["@graph"]) && (a = a["@graph"].find((e => (e["@type"] || "").match(this.REGEXPS.jsonLdArticleTypes)))), !a || !a["@type"] || !a["@type"].match(this.REGEXPS.jsonLdArticleTypes)) return;
        if (t = {}, "string" == typeof a.name && "string" == typeof a.headline && a.name !== a.headline) {
          var s = this._getArticleTitle(),
            n = this._textSimilarity(a.name, s) > .75,
            l = this._textSimilarity(a.headline, s) > .75;
          t.title = l && !n ? a.headline : a.name
        } else "string" == typeof a.name ? t.title = a.name.trim() : "string" == typeof a.headline && (t.title = a.headline.trim());
        a.author && ("string" == typeof a.author.name ? t.byline = a.author.name.trim() : Array.isArray(a.author) && a.author[0] && "string" == typeof a.author[0].name && (t.byline = a.author.filter((function (e) {
          return e && "string" == typeof e.name
        })).map((function (e) {
          return e.name.trim()
        })).join(", "))), "string" == typeof a.description && (t.excerpt = a.description.trim()), a.publisher && "string" == typeof a.publisher.name && (t.siteName = a.publisher.name.trim()), "string" == typeof a.datePublished && (t.datePublished = a.datePublished.trim())
      } catch (e) {
        this.log(e.message)
      }
    })), t || {}
  ...
  _getArticleMetadata(e) {
    var t = {},
      ...
      r = /\s*(article|dc|dcterm|og|twitter)\s*:\s*(author|creator|description|published_time|title|site_name)\s*/gi,
      s = /^\s*(?:(dc|dcterm|og|twitter|parsely|weibo:(article|webpage))\s*[-\.:]\s*)?(author|creator|pub-date|description|title|site_name)\s*$/i;
    this._forEachNode(a, (function (e) {
      ...
      if (n) {
        var l = null,
          o = null;
        a && (l = a.match(r)) && (o = l[0].toLowerCase().replace(/\s/g, ""), i[o] = n.trim()), !l && t && s.test(t) && (o = t, n && (o = o.toLowerCase().replace(/\s/g, "").replace(/\./g, ":"), i[o] = n.trim()))
      }
    })), t.title = e.title || i["dc:title"] || i["dcterm:title"] || i["og:title"] || i["weibo:article:title"] || i["weibo:webpage:title"] || i.title || i["twitter:title"] || i["parsely-title"], t.title || (t.title = this._getArticleTitle());
    const n = "string" != typeof i["article:author"] || this._isUrl(i["article:author"]) ? void 0 : i["article:author"];
    return t.byline = e.byline || i["dc:creator"] || i["dcterm:creator"] || i.author || i["parsely-author"] || n, t.excerpt = e.excerpt || i["dc:description"] || i["dcterm:description"] || i["og:description"] || i["weibo:article:description"] || i["weibo:webpage:description"] || i.description || i["twitter:description"], t.siteName = e.siteName || i["og:site_name"], t.publishedTime = e.datePublished || i["article:published_time"] || i["parsely-pub-date"] || null, t.title = this._unescapeHtmlEntities(t.title), t.byline = this._unescapeHtmlEntities(t.byline), t.excerpt = this._unescapeHtmlEntities(t.excerpt), t.siteName = this._unescapeHtmlEntities(t.siteName), t.publishedTime = this._unescapeHtmlEntities(t.publishedTime), t
  },
  _isSingleImage(e) {
    for (; e;) {
    ...
    this._forEachNode(i, (function (t) {
      if (this._isSingleImage(t)) {
        ...
        if (a && this._isSingleImage(a)) {
          ...
          for (var s = i.getElementsByTagName("img")[0], n = 0; n < r.attributes.length; n++) {
            var l = r.attributes[n];
            if ("" !== l.value && ("src" === l.name || "srcset" === l.name || /\.(jpg|jpeg|png|webp)/i.test(l.value))) {
              if (s.getAttribute(l.name) === l.value) continue;
              var o = l.name;
              s.hasAttribute(o) && (o = "data-old-" + o), s.setAttribute(o, l.value)
            }
          }
  ...
  _isElementWithoutContent(e) {
    return !(e.nodeType !== this.ELEMENT_NODE || e.textContent.trim().length || e.children.length && e.children.length != e.getElementsByTagName("br").length + e.getElementsByTagName("hr").length)
  },
  ...
  _isPhrasingContent(e) {
    return e.nodeType === this.TEXT_NODE || this.PHRASING_ELEMS.includes(e.tagName) || ("A" === e.tagName || "DEL" === e.tagName || "INS" === e.tagName) && this._everyNode(e.childNodes, this._isPhrasingContent)
  },
  _isWhitespace(e) {
  ...
  _cleanStyles(e) {
    if (e && "svg" !== e.tagName.toLowerCase()) {
      ...
      this.DEPRECATED_SIZE_ATTRIBUTE_ELEMS.includes(e.tagName) && (e.removeAttribute("width"), e.removeAttribute("height"));
  ...
  _getRowAndColumnCount(e) {
    for (var t = 0, i = 0, a = e.getElementsByTagName("tr"), r = 0; r < a.length; r++) {
      ...
      for (var n = 0, l = a[r].getElementsByTagName("td"), o = 0; o < l.length; o++) {
        var h = l[o].getAttribute("colspan") || 0;
        h && (h = parseInt(h, 10)), n += h || 1
      }
      i = Math.max(i, n)
    }
    return {
      ...
      if ("presentation" != a.getAttribute("role"))
        if ("0" != a.getAttribute("datatable"))
          if (a.getAttribute("summary")) a._readabilityDataTable = !0;
            ...
            if (r && r.childNodes.length) a._readabilityDataTable = !0;
            else {
              if (["col", "colgroup", "tfoot", "thead", "th"].some((function (e) {
                ...
                var s = this._getRowAndColumnCount(a);
                1 != s.columns && 1 != s.rows ? s.rows >= 10 || s.columns > 4 ? a._readabilityDataTable = !0 : a._readabilityDataTable = s.rows * s.columns > 10 : a._readabilityDataTable = !1
              }
            }
          }
  ...
  _fixLazyImages(e) {
    this._forEachNode(this._getAllNodesWithTag(e, ["img", "picture", "figure"]), (function (e) {
      ...
      if (!(e.src || e.srcset && "null" != e.srcset) || e.className.toLowerCase().includes("lazy"))
        for (var n = 0; n < e.attributes.length; n++)
          if ("src" !== (r = e.attributes[n]).name && "srcset" !== r.name && "alt" !== r.name) {
            var l = null;
            if (/\.(jpg|jpeg|png|webp)\s+\d/.test(r.value) ? l = "srcset" : /^\s*\S+\.(jpg|jpeg|png|webp)\S*\s*$/.test(r.value) && (l = "src"), l)
              if ("IMG" === e.tagName || "PICTURE" === e.tagName) e.setAttribute(l, r.value);
              else if ("FIGURE" === e.tagName && !this._getAllNodesWithTag(e, ["img", "picture"]).length) {
              var o = this._doc.createElement("img");
              o.setAttribute(l, r.value), e.appendChild(o)
            }
          }
    }))
  },
  _getTextDensity(e, t) {
    ...
    return this._forEachNode(r, (e => a += this._getInnerText(e, !0).length)), a / i
  },
  _cleanConditionally(e, t) {
    this._flagIsActive(this.FLAG_CLEAN_CONDITIONALLY) && this._removeNodes(this._getAllNodesWithTag(e, [t]), (function (e) {
      ...
      if (!a) {
        var r = 0,
          s = this._getAllNodesWithTag(e, ["ul", "ol"]);
        this._forEachNode(s, (e => r += this._getInnerText(e).length)), a = r / this._getInnerText(e).length > .9
      }
      if ("table" === t && i(e)) return !1;
      if (this._hasAncestorTag(e, "table", -1, i)) return !1;
      if (this._hasAncestorTag(e, "code")) return !1;
      if ([...e.getElementsByTagName("table")].some((e => e._readabilityDataTable))) return !1;
      var n = this._getClassWeight(e);
      this.log("Cleaning Conditionally", e);
      if (n + 0 < 0) return !0;
      if (this._getCharCount(e, ",") < 10) {
        for (var l = e.getElementsByTagName("p").length, o = e.getElementsByTagName("img").length, h = e.getElementsByTagName("li").length - 100, d = e.getElementsByTagName("input").length, c = this._getTextDensity(e, ["h1", "h2", "h3", "h4", "h5", "h6"]), g = 0, _ = this._getAllNodesWithTag(e, ["object", "embed", "iframe"]), m = 0; m < _.length; m++) {
          for (var u = 0; u < _[m].attributes.length; u++)
            if (this._allowedVideoRegex.test(_[m].attributes[u].value)) return !1;
          if ("object" === _[m].tagName && this._allowedVideoRegex.test(_[m].innerHTML)) return !1;
          g++
        }
        var p = this._getInnerText(e);
        if (this.REGEXPS.adWords.test(p) || this.REGEXPS.loadingWords.test(p)) return !0;
        var N = p.length,
          f = this._getLinkDensity(e),
          E = ["SPAN", "LI", "TD"].concat(Array.from(this.DIV_TO_P_ELEMS)),
          b = this._getTextDensity(e, E),
          T = this._hasAncestorTag(e, "figure");
        var y = (() => {
          const e = [];
          return !T && o > 1 && l / o < .5 && e.push(`Bad p to img ratio (img=${o}, p=${l})`), !a && h > l && e.push(`Too many li's outside of a list. (li=${h} > p=${l})`), d > Math.floor(l / 3) && e.push(`Too many inputs per p. (input=${d}, p=${l})`), !a && !T && c < .9 && N < 25 && (0 === o || o > 2) && f > 0 && e.push(`Suspiciously short. (headingDensity=${c}, img=${o}, linkDensity=${f})`), !a && n < 25 && f > .2 + this._linkDensityModifier && e.push(`Low weight and a little linky. (linkDensity=${f})`), n >= 25 && f > .5 + this._linkDensityModifier && e.push(`High weight and mostly links. (linkDensity=${f})`), (1 === g && N < 75 || g > 1) && e.push(`Suspicious embed. (embedCount=${g}, contentLength=${N})`), 0 === o && 0 === b && e.push(`No useful content. (img=${o}, textDensity=${b})`), !!e.length && (this.log("Checks failed", e), !0)
        })();
        if (a && y) {
          for (var A = 0; A < e.children.length; A++) {
            if (e.children[A].children.length > 1) return y
          }
          let t = e.getElementsByTagName("li").length;
          if (o == t) return !1
        }
        return y
      }
  ...
  _isProbablyVisible: e => (!e.style || "none" != e.style.display) && (!e.style || "hidden" != e.style.visibility) && !e.hasAttribute("hidden") && (!e.hasAttribute("aria-hidden") || "true" != e.getAttribute("aria-hidden") || e.className && e.className.includes && e.className.includes("fallback-image")),
  parse() {
    ...
    if (this.log("Grabbed: " + a.innerHTML), this._postProcessContent(a), !i.excerpt) {
      ...
      r.length && (i.excerpt = r[0].textContent.trim())
    }
    var s = a.textContent;
    return {
